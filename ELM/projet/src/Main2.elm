    ...
    case msg of
        UserTyped txt ->
            case compareCommandes txt of
                Ok cmds -> 
                    ( { model | input = txt, allSteps = conversions_commandes_points cmds, visibleSteps = 1, error = False }, Cmd.none ) -- si la commande est bonne on met à jour le model
                Err _ -> 
...
display model = 
    ...
    svg
        ...
        , style "border" "1px solid #2a30ce", style "background" "#eceee8"
        ]
        [ polyline
            [ SvgAttr.points conversions_points_strings
            , SvgAttr.fill "none"
            , SvgAttr.stroke "#787aec" 
...
view model = 
    div [ style "display" "flex", style "flex-direction" "column", style "align-items" "center", style "padding" "40px", style "font-family" "sans-serif" ]
        ...
        , input 
            [ placeholder "Ex: repeat 360 [ forward 10 left 10 ]" 
            , value model.input
            , onInput UserTyped
